...
//   main: {
//     width: '100%',
//     maxWidth: '1200px', // ← Increased from 540px
//     margin: '0 auto',
//   },
//   slotGrid: {
//     display: 'grid',
//     gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', // ← Responsive
//     gap: '8px',
//   },
...
const Booking = () => {
  ...
  return (
    <div style={{
      ...
      <main style={styles.main}>
        {/* Back Button */}
        <button onClick={() => navigate('/home')} style={{ ...styles.backButton, color: colors.secondaryText }}>
          <ArrowLeft size={18} /> Back
        </button>
        ...
        <div style={styles.pageHeader}>
          ...
          <h1 style={{ ...styles.pageTitle, color: colors.heading }}>Book a Visit</h1>
          <p style={{ ...styles.pageSubtitle, color: colors.secondaryText }}>
            Please reserve a slot that works for you and me.
          </p>
        </div>

        {/* Success Message */}
        {isConfirmed && (
          <div style={{ ...styles.successBanner, backgroundColor: colors.successBg, color: colors.success, borderColor: colors.successBg }}>
        ...
        {/* Select Date */}
        <div style={styles.formGroup}>
          <label style={{ ...styles.label, color: colors.heading }}>
            <Calendar size={16} color={colors.secondaryText} style={styles.labelIcon} />
            Select Date
          </label>
          <div style={styles.chipRow}>
            {dates.map((date) => (
              <button
                ...
                style={{
                  ...
                  backgroundColor: selectedDate === date ? colors.primary : colors.cardBg,
                  color: selectedDate === date ? colors.btnText : colors.secondaryText,
                }}
              >
                {date}
              </button>
        ...
        <div style={styles.formGroup}>
          <label style={{ ...styles.label, color: colors.heading }}>
            <Clock size={16} color={colors.secondaryText} style={styles.labelIcon} />
            Select Time
          </label>
          <div style={styles.slotGrid}>
            {timeSlots.map((slot, index) => (
              <button
                ...
                style={{
                  ...
                  color: selectedTime === slot.time ? colors.btnText : colors.text,
                  ...(!slot.available ? { 
        ...
        <div style={styles.formGroup}>
          <label style={{ ...styles.label, color: colors.heading }}>
            <Users size={16} color={colors.secondaryText} style={styles.labelIcon} />
            Purpose
          </label>
          <div style={styles.purposeGrid}>
            {purposes.map((purpose) => {
              ...
              return (
                ...
                >
                  <Icon size={18} color={isSelected ? colors.primary : colors.tertiaryText} />
                  <span>{purpose.label}</span>
                </button>
              );
            })}
        ...
        <button
          ...
          style={{
            ...
            color: isFormComplete ? colors.btnText : colors.tertiaryText,
            cursor: isFormComplete ? 'pointer' : 'not-allowed',
            boxShadow: isFormComplete ? `0 4px 16px ${colors.primary}33` : 'none',
          }}
        >
          {isFormComplete ? (
            <>
        ...
        {/* Booking Summary */}
        {isFormComplete && !isConfirmed && (
          <div style={{ ...styles.summary, backgroundColor: colors.cardBg, borderColor: colors.border }}>
            <h4 style={{ ...styles.summaryTitle, color: colors.tertiaryText }}>Booking Summary</h4>
            <div style={styles.summaryRow}>
              <span style={{ ...styles.summaryLabel, color: colors.secondaryText }}>Date:</span>
              <span style={{ ...styles.summaryValue, color: colors.heading }}>{selectedDate}</span>
            </div>
            <div style={styles.summaryRow}>
              <span style={{ ...styles.summaryLabel, color: colors.secondaryText }}>Time:</span>
              <span style={{ ...styles.summaryValue, color: colors.heading }}>{selectedTime}</span>
            </div>
            <div style={styles.summaryRow}>
              <span style={{ ...styles.summaryLabel, color: colors.secondaryText }}>Purpose:</span>
              <span style={{ ...styles.summaryValue, color: colors.heading }}>{selectedPurpose}</span>
            </div>
          </div>
        )}
      </main>
